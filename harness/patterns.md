...
1. **Isolate writers**: one branch or worktree per agent. **The orchestrator assigns the isolation up front, in the brief** — create the worktree (or clone) before spawning and name its path in BOUNDARIES. Parallel writers briefed into one shared checkout collide even on separate branches: `git checkout`, index operations, and untracked files are per-checkout, so one agent's switch mutates another's working state mid-run (observed failure: a worker's branch pointer reset under it, and another committed a sibling's untracked files). The **single-writer rule** still holds inside each isolation unit — at most one agent may write a given path set at a time; every other parallel agent is a reader, advisor, or verifier.
