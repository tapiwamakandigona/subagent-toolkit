...
| **Foundation** | orchestrator → release-engineer | architecture | scaffolded repo, CI, and the state files (`../prompts/artifacts/project-state.md`; instantiate with `python3 harness/scripts/scaffold_project.py <dir> --name N --goal G [--features seed.json]`): `init.sh`, initial `features.json` | fresh clone + `init.sh` yields a running skeleton with passing (empty) test suite |
