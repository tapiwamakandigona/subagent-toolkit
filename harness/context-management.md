...
Both rules are machine-checkable: `harness/schemas/features.schema.json` pins the shape, and `python3 harness/scripts/check_features.py <features.json>` validates it, verifies every `passes: true` has evidence paths that exist and are non-empty, runs the milestone gate (`--gate [--milestone M]`), and diff-checks the frozen fields against the orchestrator's baseline copy (`--against baseline.json`). Keep a baseline snapshot from when the orchestrator last edited the file; QA runs the `--against` check before trusting any flipped `passes`.
